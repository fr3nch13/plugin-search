 ...
 * @param array
 */
	public function presetForm($model = false, $args = array()) 
	{
		if(!$model)
		{
		...
		$args = array_merge($this->controller->passedArgs, $args);
		foreach ($this->controller->presetVars as $field) {
			if ($this->encode == true || isset($field['encode']) && $field['encode'] == true) {
				...
				if (isset($args[$field['field']])) {
					$this->controller->passedArgs[$field['field']] = $args[$field['field']] = $this->decodeData($args[$field['field']]);
				}
			}
			...
			if ($field['type'] == 'value') {
				if (isset($args[$field['field']])) {
					if(is_string($args[$field['field']]))
					{
						$args[$field['field']] = trim($args[$field['field']]);
					}
					$data[$model][$field['field']] = $args[$field['field']];
				}
			}
 ...
 */
	public function serializeParams(&$data) 
	{
		foreach ($this->controller->presetVars as $field) {
			...
			if(isset($data[$field['field']]) and is_string($data[$field['field']]))
			{
				$data[$field['field']] = trim($data[$field['field']]);
				if(isset($field['trim']) and $field['trim'] and strpos($data[$field['field']], "\n") !== false)
				{
					$lines = explode("\n", $data[$field['field']]);
 ...
 */
	public function commonProcess($modelName = null, $options = array()) 
	{
		...
		if (!empty($this->controller->data)) {
			...
			if ($valid) {
				$passed = $this->controller->passedArgs;
